...
import edu.teco.pavos.transfer.data.ObservationDataSerializer;
...
public final class KafkaPropertiesFileManager extends PropertiesFileManager {
	...
	private void loadKafkaCoreProperties() {
		load(BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
		load(M_APPLICATION_ID_CONFIG, "m_application");
		load(M_CLIENT_ID_CONFIG, "m_client");
		load(SCHEMA_REGISTRY_URL_CONFIG, "http://localhost:8081");
	...
	public Properties getGridProducerProperties() {
    	Properties props = new Properties();
    	props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, getProperty(BOOTSTRAP_SERVERS_CONFIG));
